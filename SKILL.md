...
- **No jargon unless the user used it first.** Match their vocabulary. Technical users get technical options; non-technical users get plain descriptions.
- **Every option = a real thing.** The user should be able to picture what they'd get.

**Contradiction detection:** After each answer, cross-check against all previously locked decisions. If a contradiction or high-friction pairing is detected, surface it: "You said X for Q1 and Y for Q2 — together these mean [consequence]. Which do you want to adjust?" Do NOT silently lock contradictory pairs.

**Backtracking:** If the user revises a previous answer, update the decision, discard any downstream answers that depended on the changed decision, and re-ask only those invalidated forks. Never re-ask independent forks.

**Confusion handling:** If the user says they don't understand the options, rephrase with simpler language or concrete analogies. Re-present via `ask_user_questions`. If still confused after one rephrase, offer: "Want me to pick the safest default? You can change it later."

**Fast-path:** If the user picks Surprise Me twice consecutively, ask: "Want me to pick the best defaults for the rest too?" If yes, auto-resolve all remaining forks with `source: default` and skip to Step 5.
...
**Any agent working on this project MUST read `.zeroguess` before starting.**

The `.zeroguess` file header contains this instruction for consuming agents:
...
1. Check if it's in `decisions:` → use the locked value, no exceptions
2. Check if it's in `open_questions:` → **stop and ask the user before proceeding**
...
**Language rule:** The `.zeroguess` file is always written in English (machine-readable YAML values). The summary shown to the user uses their current language.
...
4. **Asking permission to ask.** Never say "I have a few questions", "mind if I clarify?", "before we start, can I ask...?" — just ask the first fork immediately. No preamble.
...
- Option 3: Surprise me — a real-time group chat with rooms where friends can hang out and talk
